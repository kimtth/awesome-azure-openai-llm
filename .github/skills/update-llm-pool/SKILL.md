...
description: "Workflow for updating the LLM landscape paper pool (section/x_llm_papers.md) using fetch_llm_papers.py. Covers full re-fetch, resume from checkpoint, inject-only, and adding new topics. USE FOR: Refreshing citation counts, expanding topic coverage, re-injecting top papers into models_research.md. DO NOT USE FOR: Adding hand-curated entries to section files (use add-new-entry), updating RAG/Agent citation sections in best_practices.md (use update-cite-count)."
---
...
The section `### **LLM Research (Ranked by cite count >=100)**` in `section/models_research.md` links to this file with a single descriptive line. Top-N entries can optionally be injected directly into that section.
...
| `--target-file` | *(none)* | Markdown file to inject entries into (e.g. `section/models_research.md`) |
| `--target-section` | `### **LLM Research (Ranked by cite count >=100)**` | Heading in `--target-file` to inject under |
| `--target-top-n` | `30` | How many top papers to inject into `--target-section` |
| `--reset` | *(flag)* | Delete existing checkpoint and start from scratch |
| `--inject-only` | *(flag)* | Skip fetching; re-inject from existing output file without API calls |
...
.venv\Scripts\python.exe code/fetch_llm_papers.py `
    ...
    --min-citations 150 `
    --top-n 50 `
    --target-file section/models_research.md
```
...
### 3. Inject-only (no API calls)

Use to change the number of injected entries or re-inject after a manual edit to `x_llm_papers.md`, without hitting the API:
...
- Reads `section/x_llm_papers.md` (must already exist and be sorted).
- Re-numbers entries sequentially in case the file has stale `1.` prefixes.
- Replaces the body of `--target-section` with the top N lines.
...
3. **Stale `1.` numbering:** If `x_llm_papers.md` was generated by an older version of the script, all entries start with `1.`. Use `--inject-only` — it auto-renumbers before injecting.

4. **`--inject-only` without an existing output file:** The flag requires `section/x_llm_papers.md` to already exist. Run a full fetch first if the file is missing.

5. **Wrong `--target-section` heading:** The heading must match exactly (including `### **...**` markdown). Default is `### **LLM Research (Ranked by cite count >=100)**`. If the heading is not found, the script prints `[WARN] Section not found` and skips injection without error.

6. **Non-CS papers in results:** The filter requires `"Computer Science"` in `fieldsOfStudy`. Some highly cited ML papers may not be tagged as CS by Semantic Scholar and will be excluded. This is intentional.
